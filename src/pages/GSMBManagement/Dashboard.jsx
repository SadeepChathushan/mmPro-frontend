...
import { Layout, Row, Col, Card, Typography, Divider, List, Avatar, DatePicker, Space } from "antd";
import {
  LineChart,
  Line,
  PieChart,
...
import { UserOutlined, RiseOutlined, FileTextOutlined, ExclamationCircleOutlined, MoneyCollectOutlined } from "@ant-design/icons";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchRoyaltyCounts = async () => {
      try {
        ...
        <Layout style={{ minHeight: "100vh", backgroundColor: " #1e0000" }}></Layout>
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  const colors = [
    "#FFD700", "#FF8C00", "#B8860B", "#DAA520", "#FF4500", // Dark yellow shades
    "#FF0000", "#B22222", "#8B0000", "#FF6347", "#CD5C5C", // Red shades
  ...
  const darkBlueGradient = "linear-gradient(135deg,rgb(50, 0, 0),rgb(84, 1, 1),rgb(130, 0, 0))";

  return (
    <Layout style={{ minHeight: "100vh", backgroundColor: " #1e0000" }}>
      <Header style={{ background: "rgba(0, 0, 0, 0.01)", padding: "20px", boxShadow: "0 4px 8px rgba(0, 0, 0, 0.1)", borderRadius: "8px" }}>
        ...
        <Row gutter={16} style={{ marginTop: "20px" }}>
          <Col xs={24} md={16}>
            ...
            <Card bordered={false} style={{ backgroundColor: "rgba(254, 118, 118, 0.1)", borderRadius: "8px", padding: "20px" }}>
              <Title level={5} style={{ color: "#fff", textAlign: "center" }}>Monthly Cube Count (Sand, Graphite, and Mines)</Title>
              <ResponsiveContainer width="100%" height={350}>
                <div style={{ textAlign: "center", marginBottom: "10px", color: "#fff" }}>
                  {/* Title */}

                  {/* Description */}
                  <p style={{ fontSize: "12px", color: "#ffef2f" }}>+ 50% of cube count increased</p>
                </div>
                <LineChart data={lineData}>
                  ...
                  {/* Tooltip styling */}
                  <Tooltip contentStyle={{ backgroundColor: "#333", border: "none", color: "#fff", borderRadius: "6px" }} />
  ...
  <Col xs={24} md={8}>
  <Card
  title={<span style={{ color: "#fff" }}> Top Royalty Contributors</span>} // Title in white
  style={{
...
>
  ...
  <Col xs={24} md={8}>
  <Card
  title={<span style={{ color: "#fff" }}>Annual Top Mining License Categories</span>} // Title in white
  style={{
    backgroundColor: "rgba(254, 118, 118, 0.1)",
    borderRadius: "8px",
    height: "100%", // Ensures all cards have the same height
...
>
  <div style={{ color: "#fff" }}> {/* Ensure text inside card is white */}
    <p>Sand</p>
    <Progress
      percent={100}
      success={{ percent: 75 }}
